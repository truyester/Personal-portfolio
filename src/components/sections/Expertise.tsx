  ...
  const jobs = [
    {
      ...
      period: "2020 — PRESENT",
      description: "Desarrollo de ecosistemas web de alto rendimiento utilizando React, Next.js y Node.js. Enfoque en arquitectura de sistemas escalables para pequeños negocios y marcas digitales.",
    ...
    },
    {
      ...
      company: "Independent",
      period: "2024 — PRESENT",
      description: "Traducción técnica y legal de alta precisión. Especializado en documentación de ingeniería y contratos legales, apalancando un dominio C2 del idioma inglés.",
    ...
    },
    {
      ...
      period: "2022 — 2025",
      description: "Instrucción de inglés avanzado para profesionales en finanzas y servicios. Implementación de currículos basados en Spaced Repetition y roleplays técnicos.",
  ...
  return (
    <section id="experience" className="py-32 bg-background-surface/30">
      <div className="max-w-7xl mx-auto px-8">
        ...
        {/* Timeline / Grid Modular */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 relative">
          {/* Línea decorativa vertical sutil (Drift) */}
          ...
          {jobs.map((job, index) => (
            <div 
              ...
              className={`p-8 rounded-2xl bg-background-card border border-on-surface/5 hover:border-brand-primary/20 transition-all duration-500 group shadow-2xl ${
                index % 2 !== 0 ? 'md:mt-16' : '' // Modular Drift: Una tarjeta baja más que la otra
              }`}
            >
              ...
              <div className="flex flex-wrap gap-2">
                {job.tags.map((tag, i) => (
                  <span 
                    key={i} 
                    className="px-3 py-1 rounded-full bg-on-surface/5 text-[9px] font-space text-on-surface/40 tracking-tighter uppercase border border-on-surface/5 group-hover:border-brand-primary/10 transition-colors"
                  >
                    {tag}
                  </span>
                ))}
